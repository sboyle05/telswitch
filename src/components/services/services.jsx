            ...
            {content ? (
                <>
                    ...
                    {content.map((serviceEntry, index) => {
                        // Check if serviceTitle is defined and then create the path
                        let path = '';
                        ...
                        const options = {
                            renderNode: {
                                paragraph: (node, children) => (
                                    <p
                                        className={`${index % 2 === 0 ? 'textSectionLeft' : 'textSectionRight'}`}
                                    >
                                        {children}
                                    </p>
                                ),
                        ...
                        return (
                            <section
                                ...
                                className={`individualService ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}
                            >
                                <img
                                ...
                                />
                                <div
                                    className={`textSection ${index % 2 === 0 ? 'textSectionLeft' : 'textSectionRight'}`}
                                >
                                    {/* Only render Link if path is defined */}
                                    {serviceEntry.fields.title && (
                                        <Link to={path} className={`font-bold text-lg ${index % 2 === 0 ? 'textSectionLeft' : 'textSectionRight'}`}>
                                            {serviceEntry.fields.title}
                                        </Link>
                                    )}
